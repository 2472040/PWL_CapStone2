...
export const draftItemSchema = z.object({
  ...
  name: z.string().min(3, { message: 'Nama barang minimal 3 karakter.' }).max(200, { message: 'Nama barang maksimal 200 karakter.' }),
  qty: z.number({ invalid_type_error: 'Jumlah barang harus berupa angka.' })
    .int({ message: 'Jumlah barang harus berupa angka bulat.' })
    .positive({ message: 'Jumlah barang harus lebih besar dari 0.' }),
  unit: z.string({ required_error: 'Satuan barang wajib diisi.' })
    .min(1, { message: 'Satuan barang wajib diisi.' })
    .max(50, { message: 'Satuan barang maksimal 50 karakter.' }),
  price: z.number({ invalid_type_error: 'Harga barang harus berupa angka.' })
