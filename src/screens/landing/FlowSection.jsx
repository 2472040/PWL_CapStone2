        ...
        gsap.fromTo(
          ...
          { x: startX, y: startY, opacity: 1, scale: 0.5 },
          {
            ...
            onComplete: () => {
              ...
              gsap.fromTo(
                buttons[step],
                { scale: 1.05, backgroundColor: 'rgba(255,255,255,0.2)' },
                { scale: 1, backgroundColor: 'rgba(255,255,255,0.08)', duration: 0.4 }
              );
            },
          }
        );
    ...
    <section className="au-section" id="flow-section">
      <AuRise>
        <div className="au-section-tag">
          <ScrambleText text="— Demo interaktif" />
        </div>
        <h2 className="au-section-h2">
          Satu draf <em>mengalir</em> dari satu meja ke meja berikutnya.
        </h2>
        <p className="au-section-sub">
          Klik antar role untuk melihat tampilan masing-masing dari draf yang sama. Approval,
          penolakan, dan penerimaan terpicu di tempat — bukan di email atau spreadsheet terpisah.
        </p>
      </AuRise>
      ...
      <AuRise className="au-flow-stage">
        ...
        <div className="au-flow-body">
          ...
          {step === 2 && (
            <AdminView approvals={approvals} received={received} setReceive={setReceive} />
          )}
        </div>
      </AuRise>
    </section>
      ...
      <div className="au-items">
        {D.draft.items.map((it, i) => (
          <div key={it.id} className="au-item" style={{ animationDelay: `${i * 30}ms` }}>
            <div className={`au-item-kind ${it.kind === 'Inventaris' ? 'inv' : 'bhp'}`}>
      ...
      <div className="au-draft-foot">
        <div className="au-totals">
          <div>
            ...
            <div className="au-total-val violet">
              {fmtRpShort(
                D.draft.items
                  .filter((i) => i.kind === 'Inventaris')
          ...
          <div>
            ...
            <div className="au-total-val cyan">
              {fmtRpShort(
                D.draft.items
                  .filter((i) => i.kind === 'BHP')
                  .reduce((s, i) => s + i.qty * i.price, 0)
          ...
          <div>
            ...
            <div className="au-total-val">
              {fmtRpShort(D.draft.items.reduce((s, i) => s + i.qty * i.price, 0))}
            </div>
          </div>
        </div>
      ...
      <div className="au-items">
        {D.draft.items.map((it) => {
          ...
          return (
            <div
              key={it.id}
              className={`au-item ${st === 'ok' ? 'approved' : ''} ${st === 'no' ? 'rejected' : ''}`}
            >
              <div className={`au-item-kind ${it.kind === 'Inventaris' ? 'inv' : 'bhp'}`}>
                {it.kind === 'Inventaris' ? 'Inv' : 'BHP'}
              </div>
              <div>
                ...
                <div className="au-item-name-sub">
                  ...
                  {it.replaces && (
                    <>
                      <span>·</span>
                      <span className="replaces">↺ {it.replaces}</span>
                    </>
                  )}
                </div>
              </div>
    ...
    <div>
      <div className="au-draft-head">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button className="au-btn-primary" style={{ padding: '8px 14px', fontSize: 12 }}>
            Export ke aset
          </button>
        </div>
      </div>
      ...
      <div className="au-admin-grid">
        {eligible.map((it) => {
          ...
          return (
            <div
              key={it.id}
              className="au-inv-card"
              style={{ padding: 16, opacity: isRec ? 1 : 0.85 }}
            >
              <div className="au-inv-card-head">
                <div>
                  <div
                    className={`au-item-kind ${it.kind === 'Inventaris' ? 'inv' : 'bhp'}`}
              ...
              <div style={{ fontSize: 14, fontWeight: 500, lineHeight: 1.3, marginBottom: 6 }}>
                {it.name}
              </div>
