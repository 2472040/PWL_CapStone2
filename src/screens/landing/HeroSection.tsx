  ...
  return (
    <section className="au-hero" id="hero">
      ...
      <p className="au-hero-sub">
        Sistem terintegrasi untuk Kepala Lab, Kaprodi, Staf Administrasi, dan Staf Lab — dari draf
        pengadaan tahunan, approval per-item, sampai log maintenance dan stok BHP yang menyusut
        otomatis.
      </p>
      <div className="au-hero-ctas" ref={ctaRef}>
        <ShinyButton onClick={onEnterApp}>Sign In &rarr;</ShinyButton>
      </div>

      <div style={{ position: 'relative' }}>
        <div className="au-hero-glow">
          <div className="au-hero-glow-orb" />
        </div>
        <div className="au-hero-preview" ref={previewRef}>
          <div className="au-preview-bar">
            ...
            >
              <div
                style={{ cursor: 'pointer', color: tab === 'draft' ? 'var(--cyan)' : 'inherit' }}
              ...
              <div
                style={{
                  cursor: 'pointer',
                  color: tab === 'inventory' ? 'var(--cyan)' : 'inherit',
                }}
              ...
              <div
                style={{ cursor: 'pointer', color: tab === 'activity' ? 'var(--cyan)' : 'inherit' }}
          ...
          <div
            className="au-preview-body"
            ref={bodyRef}
            style={{ minHeight: '260px', transition: 'background 0.3s' }}
          >
            {tab === 'draft' && (
              <>
                ...
                >
                  <div className="au-preview-stat" style={{ padding: 16 }}>
                    ...
                    >
                      ...
                      <div className="au-mono" style={{ fontSize: 11, color: 'var(--ink-3)' }}>
                        21 Apr — 28 Apr
                      </div>
                    </div>
                    <Sparkline />
                  ...
                  <div className="au-preview-stat" style={{ padding: 14 }}>
                    ...
                    {D.rooms.slice(0, 3).map((r) => (
                      ...
                      >
                        ...
                        <span className="au-mono" style={{ color: 'var(--ink-3)' }}>
                          {r.count}
                        </span>
                      </div>
                    ))}
            ...
            {tab === 'inventory' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={{ fontSize: 13, color: 'var(--ink-2)', marginBottom: '8px' }}>
                  Daftar Aset Terkini
                </div>
                  ...
                  >
                    ...
                    >
                      <span
                        style={{
                          fontSize: 11,
                          color: inv.cond === 'Baik' ? 'var(--green)' : 'var(--amber)',
                        }}
                      >
                        {inv.cond}
                      </span>
                      <span className="au-mono" style={{ fontSize: 11, color: 'var(--ink-3)' }}>
                        {inv.code}
                      </span>
                    </div>
                  </div>
            ...
            {tab === 'activity' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ fontSize: 13, color: 'var(--ink-2)', marginBottom: '4px' }}>
                ...
                {D.activity.slice(0, 4).map((act, i) => (
                  ...
                  >
                    <div style={{ marginTop: '2px', color: 'var(--cyan)' }}>
                      <RoleIcon kind={act.role.toLowerCase().replace(' ', '')} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, color: 'var(--ink)' }}>
                        <strong>{act.who}</strong> {act.act}{' '}
                        <span style={{ color: 'var(--ink-2)' }}>{act.target}</span>
                      </div>
                      <div
                        className="au-mono"
                        style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: '4px' }}
                      >
                        {act.when}
                      </div>
                    </div>
