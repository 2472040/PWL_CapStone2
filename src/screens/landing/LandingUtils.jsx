...
export const D = {
  roles: [
    { id:'kalab', title:'Kepala Laboratorium', short:'Kalab', action:'Susun draf pengadaan', desc:'Rencanakan kebutuhan tahunan — inventaris baru, BHP, dan penggantian aset lama.', tasks:['Buat draf pengadaan tahunan','Tandai aset yang diganti','Lihat riwayat draf'], color:'aurora' },
    { id:'kaprodi', title:'Ketua Program Studi', short:'Kaprodi', action:'Review & approval', desc:'Tinjau draf, setujui atau tolak per item, finalisasi pengadaan.', tasks:['Approve per item','Komentar revisi','Finalisasi (lock)'], color:'azure' },
    { id:'admin', title:'Staf Administrasi', short:'Admin', action:'Receive & label', desc:'Catat penerimaan, generate kode QR/Barcode, tempel label fisik.', tasks:['Tanggal penerimaan','Generate QR','Penomoran label'], color:'lime' },
    { id:'staflab', title:'Staf Laboratorium', short:'Staf Lab', action:'Maintenance & stok', desc:'Log pemeliharaan, update kondisi, kelola stok BHP.', tasks:['Log maintenance','Update kondisi','Decrement BHP otomatis'], color:'amber' },
    { id:'sysadmin', title:'Administrator', short:'Sys Admin', action:'User & ruangan', desc:'Kelola akun pengguna, ruangan lab, dan permission.', tasks:['Manage users','Manage rooms','Access control'], color:'slate' },
  ],
  draft: {
    code:'PRC-2026-LK01', title:'Pengadaan Lab Komputer · 2026', by:'Dr. Pradipta Wirasena',
    role:'Kepala Laboratorium · Informatika', submitted:'14 Mei 2026',
    items: [
      { id:'I-01', kind:'Inventaris', name:'Workstation Dev — Ryzen 9 / 64GB / RTX 4070', qty:12, unit:'unit', price:32_500_000, link:'shopee.co.id/...', replaces:'PC-LK-2019 (12 unit, kondisi rusak)' },
      { id:'I-02', kind:'Inventaris', name:'Monitor IPS 27" 1440p · 144Hz', qty:12, unit:'unit', price:4_200_000, link:'tokopedia.com/...', replaces:null },
      { id:'I-03', kind:'Inventaris', name:'Managed Switch 24-Port Gigabit', qty:2, unit:'unit', price:8_900_000, link:'distributor-net.id/...', replaces:'SW-LK-2017 (Cisco SG200, EOL)' },
      { id:'B-01', kind:'BHP', name:'Kabel UTP Cat6 Belden · 305m roll', qty:3, unit:'roll', price:2_750_000, link:'distributor-net.id/...', replaces:null },
      { id:'B-02', kind:'BHP', name:'Konektor RJ45 Cat6 (100pcs)', qty:10, unit:'pack', price:165_000, link:'tokopedia.com/...', replaces:null },
  ...
  stats: { totalAssets:287, activeDrafts:3, bhpItems:142, rooms:9, inMaintenance:7, pendingApproval:14 },
  activity: [
...
export function AuRise({ children, as: As = 'div', className = '', ...rest }) {
  return <As className={`au-rise ${className}`} {...rest}>{children}</As>;
}
export function AuStagger({ children, as: As = 'div', className = '', ...rest }) {
  return <As className={`au-stagger ${className}`} {...rest}>{children}</As>;
}
