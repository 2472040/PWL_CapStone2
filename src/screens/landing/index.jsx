...
export default function AuroraSite({ onEnterApp }) {
  ...
  const setApproval = (id, val) => setApprovals(p => ({ ...p, [id]: p[id] === val ? null : val }));
  const approveAll = () => {
    const o = {}; D.draft.items.forEach(it => o[it.id] = 'ok'); setApprovals(o);
  };
  const toggleReceive = (id) => setReceive(p => ({ ...p, [id]: !p[id] }));

  const totals = {
    all: D.draft.items.reduce((s, i) => s + i.qty * i.price, 0),
    approved: D.draft.items.filter(it => approvals[it.id] === 'ok').reduce((s, i) => s + i.qty * i.price, 0),
  };
  ...
  useEffect(() => {
    ...
    let ctx = gsap.context(() => {
      ...
      tl.fromTo('.au-nav', { y: -20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8, ease: 'power3.out' })
        .fromTo('.au-eyebrow', { opacity: 0, y: 10 }, { opacity: 1, y: 0, duration: 0.5 }, '-=0.4')
        .fromTo('.au-h1', { opacity: 0, y: 30, rotationX: -15, transformPerspective: 800 }, { opacity: 1, y: 0, rotationX: 0, duration: 1, ease: 'power4.out' }, '-=0.2')
        .fromTo('.au-hero-sub', { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.8, ease: 'power3.out' }, '-=0.6')
        .fromTo('.au-hero-ctas > *', { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.6, stagger: 0.1 }, '-=0.5')
        .fromTo('.au-hero-preview', 
        ...
        .fromTo('.au-trusted-label', { opacity: 0, y: 10 }, { opacity: 1, y: 0, duration: 0.5 }, '-=0.3')
        .fromTo('.au-trusted-item', { opacity: 0, y: 15 }, { opacity: 0.5, y: 0, stagger: 0.08, duration: 0.5 }, '-=0.3');
      ...
      root.querySelectorAll('.au-section-h2').forEach(el => {
        gsap.fromTo(el, 
          ...
          { y: 0, opacity: 1, rotationX: 0, duration: 1, ease: 'power4.out', scrollTrigger: { trigger: el, start: 'top 85%' } }
        );
      });
      ...
      gsap.fromTo('.au-flow-stage', 
        ...
        { y: 0, opacity: 1, scale: 1, duration: 1, ease: 'power4.out', scrollTrigger: { trigger: '.au-flow-stage', start: 'top 85%' } }
      );

      // Flow steps
      gsap.fromTo('.au-flow-step', 
        { x: -20, opacity: 0 }, 
        { x: 0, opacity: 1, stagger: 0.1, duration: 0.5, scrollTrigger: { trigger: '.au-flow-bar', start: 'top 85%' } }
      );

      // Draft items stagger
      gsap.fromTo('.au-item', 
        { x: -30, opacity: 0 }, 
        { x: 0, opacity: 1, stagger: 0.08, duration: 0.6, ease: 'power2.out', scrollTrigger: { trigger: '.au-items', start: 'top 85%' } }
      );

      // Draft totals
      gsap.fromTo('.au-totals > div', 
        { y: 20, opacity: 0 }, 
        { y: 0, opacity: 1, stagger: 0.1, duration: 0.5, scrollTrigger: { trigger: '.au-draft-foot', start: 'top 95%' } }
      );

      // Inventory cards (Scroll-Pinned Feature Build-up)
      const invSection = root.querySelector('#inv-section');
      if (invSection) {
        gsap.fromTo('.au-inv-card', 
      ...
      // Activity rows
      gsap.fromTo('.au-activity-row', 
        { x: 30, opacity: 0 }, 
        { x: 0, opacity: 1, stagger: 0.1, duration: 0.6, ease: 'power2.out', scrollTrigger: { trigger: '.au-activity', start: 'top 85%' } }
      );

      // Stats counter cards
      gsap.fromTo('.au-stat-item',
        { y: 40, opacity: 0, scale: 0.9 },
        { y: 0, opacity: 1, scale: 1, stagger: 0.1, duration: 0.7, ease: 'back.out(1.2)', scrollTrigger: { trigger: '#stats-section', start: 'top 85%' } }
      );

      // Testimonial card
      gsap.fromTo('.au-testimonial-card',
        { y: 50, opacity: 0, scale: 0.95 },
        { y: 0, opacity: 1, scale: 1, duration: 1, ease: 'power3.out', scrollTrigger: { trigger: '#testimonials-section', start: 'top 80%' } }
      );

      // CTA section timeline
      const ctaTl = gsap.timeline({ scrollTrigger: { trigger: '.au-cta', start: 'top 80%' } });
      ctaTl.fromTo('.au-cta', { y: 40, opacity: 0, scale: 0.97 }, { y: 0, opacity: 1, scale: 1, duration: 0.8, ease: 'power3.out' })
           ...
           .fromTo('.au-cta-btns > *', { y: 16, opacity: 0 }, { y: 0, opacity: 1, stagger: 0.1, duration: 0.5 }, '-=0.2');

      // Footer
      gsap.fromTo('.au-foot', 
        { y: 20, opacity: 0 }, 
        { y: 0, opacity: 1, duration: 0.6, scrollTrigger: { trigger: '.au-foot', start: 'top 95%' } }
      );
    ...
    <div className="au" ref={rootRef}>
      ...
      <div className="au-layer">
        ...
        <FlowSection
          ...
          received={received} setReceive={toggleReceive}
          totals={totals}
        />
        <InventorySection />
        <BentoFeatures />
