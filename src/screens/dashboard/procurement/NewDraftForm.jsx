...
export function NewDraftForm({ close }) {
  ...
  return (
    <>
      <div className="drawer-bar"><div className="drawer-title">Draf pengadaan baru</div><button className="x-btn" onClick={close} disabled={loading}><Icon name="x" size={14} /></button></div>
      <div className="drawer-body">
        <div className="field"><div className="field-lbl">Judul draf <span className="req">*</span></div><input className="input" value={title} onChange={e => setTitle(e.target.value)} disabled={loading} /></div>

        <div className="flex between aic mt-4 mb-3">
          <div className="field-lbl m-0" >Item barang ({items.length})</div>
          <button className="btn sm" onClick={add} disabled={loading}><Icon name="plus" size={11} /> Item</button>
        </div>
      ...
      <div className="drawer-foot">
        <button className="btn" onClick={() => handleSave(false)} disabled={loading}>Simpan draft</button>
        <button className="btn primary" onClick={() => handleSave(true)} disabled={loading}>
          {loading ? 'Memproses...' : <><Icon name="arrow" size={13} /> Ajukan ke Kaprodi</>}
        </button>
      </div>
    </>
  );
