    ...
    <div className="page" style={{ '--role-accent': role.accent }}>
      ...
      {inbox.length === 0 ? (
        ...
        <div className="flex flex-col gap-4" data-reveal-children>
          {inbox.map(d => (
            <ReviewCard key={d.code} d={d} accent={role.accent} toast={toast} dispatch={dispatch} onOpen={() => setOpenCode(d.code)} />
          ))}
        </div>
      )}
    </div>
...
function ReviewCard({ d, accent, toast, dispatch, onOpen }) {
  ...
  async function handleApproveReject(itemId, status) {
    try {
      ...
      dispatch({ type: 'ITEM_APPROVAL', draftId: d.id, itemId, status: status === 'approved' ? 'ok' : 'no' });
    } catch (err) {
  ...
  return (
    <div className="card" style={{'--role-accent': accent, position: 'relative', overflow: 'hidden'}}>
      <div style={{position: 'absolute', top: 0, left: 0, width: '4px', height: '100%', background: 'var(--role-accent)'}} />
      <div className="flex between aic mb-4">
        <div>
          <div className="flex gap-2 items-center mb-1">
            <span className="mono text-xs fw-6 text-3">{d.code}</span>
            <span className="chip warn"><span className="dot" /> Menunggu review</span>
          </div>
          <h3 className="text-xl fw-5 tracking-tight cursor-pointer hover:underline" onClick={onOpen}>{d.title}</h3>
          <div className="text-sm text-ink-3 mt-1">Diajukan oleh: <b>{d.by}</b> pada {d.submitted_at ? new Date(d.submitted_at).toLocaleDateString('id-ID', {day:'numeric',month:'short',year:'numeric'}) : ''}</div>
        </div>
        <div className="text-right">
          <div className="text-xs text-3 mono mb-1">TOTAL AJUAN</div>
          <div className="text-xl fw-6 font-mono tracking-tight text-ink">Rp {formatThousand(totals.all)}</div>
        </div>
      </div>
              ...
              <tr key={it.id} className="hover:bg-surface-2 transition-colors">
                <td className="px-3 py-2.5">
                  ...
                  <div className="text-xs text-3 flex gap-2 items-center mt-0.5">
                    ...
                    {it.link && <a href={it.link} target="_blank" rel="noreferrer" className="text-accent hover:underline flex items-center gap-1"><Icon name="external-link" size={10} /> Link</a>}
                  </div>
                </td>
                <td className="px-3 py-2.5 mono text-xs">Rp {formatThousand(it.price)}</td>
                <td className="px-3 py-2.5 text-center">{it.qty}</td>
                <td className="px-3 py-2.5 mono text-xs text-right fw-5">Rp {formatThousand(it.qty * it.price)}</td>
                <td className="px-3 py-2.5">
                  {it.approval === 'ok' ? (
                    <div className="flex items-center justify-center gap-1.5 text-emerald-500 text-xs fw-5"><Icon name="check" size={12} /> Disetujui</div>
                  ) : it.approval === 'no' ? (
                    <div className="flex items-center justify-center gap-1.5 text-rose-500 text-xs fw-5"><Icon name="x" size={12} /> Ditolak</div>
                  ) : (
                    <div className="flex gap-1.5 justify-center">
                      <button className="btn sm ok w-full justify-center" onClick={() => handleApproveReject(it.id, 'approved')} disabled={loading} title="Setujui">
                        <Icon name="check" size={12} />
                      </button>
                      <button className="btn sm danger w-full justify-center" onClick={() => handleApproveReject(it.id, 'rejected')} disabled={loading} title="Tolak">
                        <Icon name="x" size={12} />
                      </button>
                    </div>
                  )}
      ...
      <div className="flex justify-between items-center pt-2 border-t border-line-2">
        <div className="text-sm">
          <span className="text-emerald-500 fw-6">{totals.ok} disetujui</span> · <span className="text-rose-500 fw-6">{totals.no} ditolak</span> · <span className="text-amber-500 fw-6">{totals.pending} pending</span>
        </div>
        <div className="flex gap-2">
          ...
          <button className={`btn ${totals.pending === 0 ? 'primary' : ''}`} onClick={finalize} disabled={loading || totals.pending > 0}>
