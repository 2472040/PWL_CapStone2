...
export function AiPredictiveModal({ payload, close }) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center" style={{ minWidth: '400px', minHeight: '300px' }}>
        ...
        <p className="text-xs text-ink-3 mt-1">Mengumpulkan riwayat transaksi pengadaan & log pemeliharaan.</p>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center" style={{ minWidth: '400px', minHeight: '300px' }}>
        ...
        <p className="text-xs text-ink-3 mt-1">Gagal memproses model analisis prediktif pada server lokal.</p>
        <button className="btn sm mt-4" onClick={close}>Tutup</button>
      </div>
    );
  }
      ...
      {training ? (
        ...
        <div className="flex flex-col items-center justify-center py-10 text-center flex-1">
          <div className="relative w-16 h-16 flex items-center justify-center mb-5">
            ...
            <Icon name="bolt" size={24} className="text-violet animate-pulse" />
          </div>
          
          <h3 className="text-sm font-bold tracking-tight">Melatih Model Regresi Linear...</h3>
          
          <div className="w-full bg-surface-2 rounded-full h-1 mt-4 max-w-[200px] overflow-hidden">
            <div className="bg-violet h-full rounded-full transition-all duration-100" style={{ width: `${(epoch / data.epochsTrained) * 100}%` }} />
          </div>

          <div className="flex gap-4 justify-center items-center mt-6 mono text-[11px] text-ink-3">
            <div>Epoch: <span className="text-violet font-semibold">{epoch}</span>/{data.epochsTrained}</div>
            <div className="h-3 w-px bg-surface" />
            <div>Loss (MSE): <span className="text-cyan font-semibold">{currentLoss}</span></div>
          </div>
        </div>
      ) : (
        ...
        <div className="flex flex-col gap-4 flex-1">
          ...
          <div className="card compact glow flex items-start gap-3 p-4" style={{ '--role-accent': predictedDays < 15 ? 'var(--color-rose)' : 'var(--color-cyan)', background: 'rgba(255,255,255,0.01)' }}>
            ...
            <div>
              <span className="text-[10px] text-ink-3 uppercase tracking-wider font-bold">Hasil Estimasi AI</span>
              ...
              <p className="text-[11px] text-ink-3 mt-1.5 leading-normal">
                {predictedDays > 0 && predictedDays < 365 ? (
                  <>Berdasarkan tren penggunaan harian, sisa <b>{currentStock} {unit}</b> diprediksi akan habis pada tanggal <b>{new Date(predictedDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</b>.</>
                ) : (
                  <>Rata-rata penggunaan harian sangat kecil. Stok <b>{currentStock} {unit}</b> saat ini diproyeksikan mencukupi kebutuhan jangka panjang.</>
                )}
              </p>
            </div>
          </div>
          ...
          <div className="card compact p-3 flex flex-col items-center">
            <span className="text-[10px] text-ink-3 uppercase tracking-wider font-bold mb-2 self-start">Visualisasi Tren & Regresi Linear</span>
            
            {hasCoordinates ? (
              <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto overflow-visible mt-1">
                ...
                {/* Vertical Axis line */}
                <line x1={padding} y1={padding} x2={padding} y2={height - padding} stroke="rgba(255,255,255,0.1)" strokeWidth={1.5} />

                {/* Regression fitted line */}
                {regressionPath && (
                  <path d={regressionPath} fill="none" stroke="var(--color-violet)" strokeWidth={2} strokeDasharray="3 3" style={{ opacity: 0.8 }} />
                )}

                {/* Historical Scatter plot points */}
                {points.map((p, idx) => (
                  <g key={idx} className="group">
                    <circle cx={p.cx} cy={p.cy} r={4} fill="var(--color-cyan)" className="transition-all hover:r-6 cursor-help" />
                    <title>{`Hari ${p.x}: Pakai ${p.y} ${unit}`}</title>
                  </g>
                ))}
                
                {/* Legend / Axis Labels */}
                <text x={padding} y={height - 6} fill="rgba(255,255,255,0.3)" fontSize={8} fontFamily="monospace">WAKTU (HARI)</text>
                <text x={width - padding} y={height - 6} fill="rgba(255,255,255,0.3)" fontSize={8} fontFamily="monospace" textAnchor="end">HARI INI</text>
                <text x={padding + 5} y={padding + 8} fill="rgba(255,255,255,0.3)" fontSize={8} fontFamily="monospace" transform={`rotate(90 ${padding} ${padding})`}>KONSUMSI ({unit.toUpperCase()})</text>
              </svg>
            ) : (
              <div className="flex flex-col items-center justify-center py-8 text-center text-ink-3">
                <Icon name="cal" size={24} className="opacity-30 mb-2" />
                <span className="text-[11px] leading-relaxed">Belum ada riwayat konsumsi BHP.<br />Model menggunakan asumsi standar.</span>
              </div>
            )}
          </div>
