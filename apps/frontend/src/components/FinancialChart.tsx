...
export function FinancialChart({ data }: FinancialChartProps) {
  ...
  const maxVal = Math.max(
    ...chartData.map((d: FinancialDataPoint) => Math.max(d.requested, d.approved, 1000000))
  );
    ...
    >
      <div className="flex between aic mb-5">
        ...
        <div className="flex gap-4 text-xs font-semibold">
          <div className="flex gap-1.5 items-center">
            ...
            <span className="text-ink-3">Pengajuan</span>
          </div>
          <div className="flex gap-1.5 items-center">
            <span className="h-2.5 w-2.5 rounded-full" style={{ background: '#22c55e' }} />{' '}
            <span className="text-ink-3">Disetujui</span>
          </div>
          <div className="flex gap-1.5 items-center">
            <span className="h-2.5 w-2.5 rounded-full" style={{ background: '#eab308' }} />{' '}
            <span className="text-ink-3">Penghematan</span>
          </div>
        </div>
      </div>
      ...
      <div className="relative w-full h-[220px] select-none">
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible">
          ...
          <line
            x1={padding}
            y1={(height - 2 * padding) / 2 + padding}
            x2={width - padding}
            y2={(height - 2 * padding) / 2 + padding}
            stroke="rgba(255,255,255,0.03)"
            strokeWidth={1}
          />
          <line
          ...
          <text
            ...
            textAnchor="end"
          >
            {formatRpShort(maxVal)}
          </text>
          <text
            x={padding - 5}
            y={(height - 2 * padding) / 2 + padding + 3}
            fill="rgba(255,255,255,0.25)"
            fontSize={7.5}
            fontFamily="monospace"
            textAnchor="end"
          >
            {formatRpShort(maxVal / 2)}
          </text>
          <text
            x={padding - 5}
            y={height - padding + 3}
          ...
          <path
            d={chartData
              .map((d, i) => `${i === 0 ? 'M' : 'L'} ${getX(i)} ${getY(d.requested)}`)
              .join(' ')}
            fill="none"
            stroke="#a855f7"
            strokeWidth={2}
          ...
          <path
            d={chartData
              .map((d, i) => `${i === 0 ? 'M' : 'L'} ${getX(i)} ${getY(d.approved)}`)
              .join(' ')}
            fill="none"
            stroke="#22c55e"
            strokeWidth={2}
          ...
          {/* Data Points circles */}
          {chartData.map((d: FinancialDataPoint, i: number) => (
            <g key={i}>
              <circle
                cx={getX(i)}
                cy={getY(d.requested)}
              ...
              <circle
                cx={getX(i)}
                cy={getY(d.requested - d.approved)}
                r={activeIdx === i ? 4.5 : 3}
                fill="#eab308"
                stroke="var(--color-surface)"
              ...
              <text
                x={getX(i)}
                y={height - padding + 15}
                fill={activeIdx === i ? 'rgba(255,255,255,0.7)' : 'rgba(255,255,255,0.35)'}
                fontSize={8.5}
          ...
          >
            ...
            <div className="flex justify-between items-center text-xs gap-3">
              ...
              <span className="mono font-semibold text-ink">
                {formatRpLong(chartData[activeIdx].requested)}
              </span>
            </div>
            <div className="flex justify-between items-center text-xs gap-3">
              <span className="flex items-center gap-1.5 text-ink-2">
              ...
              <span className="mono font-semibold text-green">
                {formatRpLong(chartData[activeIdx].approved)}
              </span>
            </div>
            <div className="flex justify-between items-center text-xs gap-3 border-t border-surface pt-1.5 mt-0.5">
              ...
              <span className="mono font-semibold text-gold">
                {formatRpLong(chartData[activeIdx].saved)}
              </span>
            </div>
          </div>
        )}
