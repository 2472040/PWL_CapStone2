...
// =========================================================
export function useKeyboardShortcuts(dispatch: (action: AppAction) => void) {
  useEffect(() => {
      ...
      for (const mutation of mutations) {
        for (const node of Array.from(mutation.addedNodes)) {
          ...
          if (el.matches?.(SELECTORS)) {
            ...
            el.querySelectorAll(SELECTORS).forEach((subEl) => {
              const htmlEl = subEl as HTMLElement;
              if (!animated.has(htmlEl)) newNodes.push(htmlEl);
            });
          }
        }
      }
