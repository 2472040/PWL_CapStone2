...
interface PredictionData {
  ...
  slopeSe: number;
  pValueScore: number;
  confLowerDays: number;
  confUpperDays: number;
...
export function AiPredictiveModal({ payload, close }: AiPredictiveModalProps) {
  ...
  let points: FittedPoint[] = [];
  let regressionPath = '';

  if (hasCoordinates) {
    const maxX = Math.max(30, ...coordinates.map((c: Coordinate) => c.x));
    const maxY = Math.max(10, ...coordinates.map((c: Coordinate) => c.y), currentStock * 1.5);

    const getX = (val: number) => padding + (val / maxX) * (width - 2 * padding);
    const getY = (val: number) => height - padding - (val / maxY) * (height - 2 * padding);

    points = coordinates.map((c: Coordinate) => ({
      cx: getX(c.x),
      cy: getY(c.y),
      x: c.x,
      y: c.y,
            ...
            <div className="card compact p-3 flex flex-col items-center">
              ...
              {hasCoordinates ? (
                ...
                >
                  ...
                  {points.map((p: FittedPoint, idx: number) => (
