...
async function seed() {
    try {
        ...
        const hashed = await bcryptjs_1.default.hash(DEFAULT_PASSWORD, 10);
        // =============================================
        // 1. USERS
        // =============================================
        ...
        // 2. ROOMS
        // =============================================
        const rooms = await models_1.Room.bulkCreate([
            {
                code: 'R-301',
                name: 'Lab Algoritma',
                floor: 3,
        ...
        const inventoryItems = await models_1.Inventory.bulkCreate(inventoryData.map((i) => ({ ...i, last_checked: new Date() })));
        console.log(`✅ ${inventoryItems.length} inventory items seeded`);
        const inv = (code) => inventoryItems.find((x) => x.code === code);
        // =============================================
        // 4. BHP
        // =============================================
        const bhpItems = await models_1.Bhp.bulkCreate([
            {
                code: 'B-001',
        ...
        // 5. DRAFTS & ITEMS
        // =============================================
        const draft1 = await models_1.Draft.create({
            code: 'PRC-2026-LK01',
        ...
        await models_1.DraftItem.bulkCreate([
            {
                draft_id: draft1.id,
                kind: 'Inventaris',
        ...
        const draft2 = await models_1.Draft.create({
            code: 'PRC-2026-LK02',
        ...
        const d2Items = await models_1.DraftItem.bulkCreate([
            {
                draft_id: draft2.id,
                kind: 'Inventaris',
        ...
        const draft3 = await models_1.Draft.create({
            code: 'PRC-2025-LK04',
        ...
        await models_1.DraftItem.create({
            draft_id: draft3.id,
            kind: 'Inventaris',
            name: 'Router Mikrotik CCR2004',
            qty: 1,
        ...
        // 6. MAINTENANCE LOGS
        // =============================================
        const m1 = await models_1.MaintenanceLog.create({
            code: 'M-2026-014',
        ...
        await models_1.MaintenanceBhp.create({
            maintenance_log_id: m1.id,
            bhp_id: b('B-009').id,
            qty_used: 0.15,
        });
        const m2 = await models_1.MaintenanceLog.create({
            code: 'M-2026-013',
        ...
        const m3 = await models_1.MaintenanceLog.create({
            code: 'M-2026-012',
        ...
        await models_1.MaintenanceBhp.create({ maintenance_log_id: m3.id, bhp_id: b('B-010').id, qty_used: 1 });
        const m4 = await models_1.MaintenanceLog.create({
            code: 'M-2026-011',
        ...
        await models_1.MaintenanceBhp.create({ maintenance_log_id: m4.id, bhp_id: b('B-003').id, qty_used: 1 });
        await models_1.MaintenanceBhp.create({
        ...
        // 7. AUDIT LOGS
        // =============================================
        await models_1.AuditLog.bulkCreate([
            {
