...
exports.calculateBhpPrediction = calculateBhpPrediction;
const models_1 = require("../models");
const errors_1 = require("../utils/errors");
async function calculateBhpPrediction(bhpId) {
    const bhp = await models_1.Bhp.findByPk(bhpId);
    if (!bhp)
        throw new errors_1.NotFoundError('BHP tidak ditemukan.');
    // Fetch all maintenance logs that used this BHP
    const usages = await models_1.MaintenanceBhp.findAll({
        where: { bhp_id: bhpId },
        include: [
            {
    ...
    if (usagesCount >= 2) {
        ...
        const firstDate = new Date(sortedUsages[0].MaintenanceLog?.date || sortedUsages[0].created_at).getTime();
        const lastDate = new Date(sortedUsages[sortedUsages.length - 1].MaintenanceLog?.date ||
            sortedUsages[sortedUsages.length - 1].created_at).getTime();
    ...
    return {
        ...
        predictedDate: predictedDays > 0 && predictedDays < 365
