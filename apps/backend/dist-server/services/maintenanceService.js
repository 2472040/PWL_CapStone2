...
async function createMaintenanceLog({ inventory_ids, action, condition_after, date, bhp_used, userId, }) {
    ...
    try {
        ...
        const lastLog = await models_1.MaintenanceLog.findOne({
            where: {
                code: {
                    [sequelize_1.Op.like]: `M-${year}-%`,
                },
            },
            order: [['code', 'DESC']],
            transaction: t,
        ...
        for (let i = 0; i < inventory_ids.length; i++) {
            ...
            const inventory = await models_1.Inventory.findByPk(inv_id, { transaction: t });
            if (!inventory)
                continue;
            inventoryCodes.push(inventory.code);
            const code = `M-${year}-${String(startCount + logsCreated.length).padStart(3, '0')}`;
            const log = await models_1.MaintenanceLog.create({
                code,
                inventory_id: inv_id,
                tech_user_id: userId,
                action,
        ...
        if (bhp_used && bhp_used.length > 0) {
            ...
            for (const item of bhp_used) {
                ...
                if (log && qtyNum > 0) {
                    ...
                    if (!totalBhpUsage[bhpIdStr])
                        totalBhpUsage[bhpIdStr] = 0;
                    totalBhpUsage[bhpIdStr] += qtyNum;
                }
            }
            for (const bhp_id of Object.keys(totalBhpUsage)) {
                const bhp = await models_1.Bhp.findByPk(bhp_id, {
                    transaction: t,
                    lock: t.LOCK.UPDATE,
                });
                if (bhp) {
