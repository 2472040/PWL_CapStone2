...
const loginRateLimiter = async (req, res, next) => {
    ...
    if (useRedis) {
        try {
            ...
            if (ipAttemptsCount >= maxAttempts) {
                const oldestIpAttempt = await redis_1.default.zrange(ipKey, 0, 0);
            ...
            if (userKey) {
                await redis_1.default.zremrangebyscore(userKey, 0, now - timeframe);
                userAttemptsCount = await redis_1.default.zcard(userKey);
                if (userAttemptsCount >= maxAttempts) {
                    const oldestUserAttempt = await redis_1.default.zrange(userKey, 0, 0);
            ...
            if (recentAttemptsCount > 0 && process.env.NODE_ENV !== 'test') {
                ...
                console.log(`[Rate Limiter - Redis] Introducing progressive delay of ${delayMs}ms for ${cleanEmail || 'unknown'} from ${ip}`);
            ...
            console.warn('[Rate Limiter] Redis error in rate limiter, using in-memory fallback:', err.message);
    ...
    if (recentAttemptsCount > 0 && process.env.NODE_ENV !== 'test') {
        const delayMs = Math.min(recentAttemptsCount * 1000, 5000);
        console.log(`[Rate Limiter - Local Memory] Introducing progressive delay of ${delayMs}ms for ${cleanEmail || 'unknown'} from ${ip}`);
        await new Promise((resolve) => setTimeout(resolve, delayMs));
    }
    // Track attempt
    ipHistory.push(now);
...
const clearLoginAttempts = (ip, email) => {
    ...
    if (useRedis) {
        try {
            ...
            const userKey = cleanEmail ? `rate:user:${cleanEmail}` : null;
            if (ip)
                redis_1.default.del(ipKey).catch(() => { });
            if (userKey)
                redis_1.default.del(userKey).catch(() => { });
        }
        catch (err) {
