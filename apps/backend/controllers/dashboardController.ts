import { User, Room, Inventory, Bhp, Draft, MaintenanceLog, AuditLog } from '../models';
...
export const getDashboardStats = asyncHandler(async (req: any, res: any) => {
  ...
  if (finalizedDrafts.length > 0) {
    const totalMs = finalizedDrafts.reduce((sum: number, d: any) => {
  ...
  stats.top3LowBhp = top3LowBhp.map((b: any) => {
  ...
  if (['sysadmin', 'kalab', 'admin'].includes(role)) {
    ...
    const roomMaintMap: Record<string, any> = {};
  ...
  if (['sysadmin', 'kalab', 'kaprodi', 'admin'].includes(role)) {
    ...
    const [financialRows]: [any[], any] = await sequelize.query(`
      SELECT
