...
export const generateBastPdf = asyncHandler(async (req: any, res: any) => {
  ...
  const hash = calculateDraftHash(draft, items);
...
export const verifyBastDocument = asyncHandler(async (req: any, res: any) => {
  ...
  const items = draft.items.filter((it: any) => !it.approval || it.approval.status === 'approved');
  const computedHash = calculateDraftHash(draft, items);
