...
import app from '../app';
import { Inventory, Room, User, sequelize } from '../models';
...
describe('Inventory CRUD Integration Test', () => {
  ...
  it('should successfully perform full inventory CRUD cycle', async () => {
    ...
    const found = resList.body.data.find((item: any) => item.id === testInventoryId);
    ...
    const resListAfter = await request(app)
      .get('/api/v1/inventory')
      .set('Authorization', `Bearer ${staffToken}`);
    const foundAfter = resListAfter.body.data.find((item: any) => item.id === testInventoryId);
    ...
    const foundAfterRestore = resListAfterRestore.body.data.find(
      (item: any) => item.id === testInventoryId
    );
