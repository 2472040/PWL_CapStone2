...
import app from '../app';
import { Draft, DraftItem, DraftApproval, sequelize } from '../models';

describe('Procurement Workflow Integration Test', () => {
  let kalabToken: string | null = null;
  let kaprodiToken: string | null = null;
  ...
  it('should successfully run a full procurement lifecycle', async () => {
    ...
    const found = resPending.body.data.find((d: any) => d.id === createdDraftId);
    ...
    const decisions = draftWithItems.items.map((it: any) => ({
      item_id: it.id,
      status: 'approved',
